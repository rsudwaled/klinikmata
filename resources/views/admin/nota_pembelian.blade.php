...
@section('content')
    ...
    <x-adminlte-modal id="modal" title="Nota Pembelian Barang" size="xl" theme="success" v-centered>
        <form action="" id="form">
            @csrf
            <div class="row">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    @php
                        $config = ['format' => 'YYYY-MM-DD'];
                    @endphp
                    <x-adminlte-input-date name="tanggal_expire" label="Tanggal Expiered" igroup-size="sm" :config="$config"
                        required />
                </div>
            </div>
        </form>
    ...
    <script>
        $(function() {
            ...
            $('.btnEdit').click(function() {
                var id = $(this).data('id');
                $.LoadingOverlay("show");
                $.get("{{ route('notapembelian.index') }}" + '/' + id, function(data) {
                    console.log(data);
                    $('#id').val(data.id);
                    $('#kode').val(data.kode);
            ...
            $('#btnStore').click(function(e) {
                $.LoadingOverlay("show");
                e.preventDefault();
                var url = "{{ route('notapembelian.store') }}";
                $.ajax({
                    data: $('#form').serialize(),
                    url: url,
                    type: "POST",
            ...
            $('#btnUpdate').click(function(e) {
                ...
                $.LoadingOverlay("show");
                e.preventDefault();
                var url = "{{ route('notapembelian.index') }}/" + id;
                $.LoadingOverlay("hide");
                $.ajax({
                    data: $('#form').serialize(),
                    url: url,
